  ...
  #define LEFT_INTERSECTION  2
  #define RIGHT_INTERSECTION 3
  ...
  float prev_weight = 0;
  float curr_weight = 0;
  unsigned int turning_sharp = 0; // 1: left turn, 2: right turn, 3: left intersection, 4: right intersection
  double detected_time = -1;
  ...
  float Position() {
    if ((filted[0] == 1) && (filted[1] == 1) && (filted[2] == 1)) {
      turning_sharp = 1;
      printf("\n\t\tSharp turn left detected");
      return weights[0];
    }
    else if ((filted[7] == 1) & (filted[6] == 1) && (filted[5] == 1)) {
      turning_sharp = 2;
      printf("\n\t\tSharp turn right detected");
      return weights[7];
    }
  ...
  void SwitchFunction() {
    switch (switch_state){
      case DEFAULT:
        switch (turning_sharp){
          case 0:
            prev_weight = curr_weight;
            curr_weight = Position();
            // Rẽ phải
            if (curr_weight > 0){
              left_ratio = base + curr_weight*weight_multiplier;
              right_ratio = base - curr_weight*weight_multiplier;
            }
            // Rẽ trái
            else if (curr_weight < 0){
              left_ratio = base + curr_weight*weight_multiplier;
              right_ratio = base - curr_weight*weight_multiplier;
            }
            // Đi thẳng
          ...
          case 1:
            left_ratio = -0.2*base;
            right_ratio = 0.6*base;
            //if (currSensors == 0b10000000 || currSensors == 0b00010000 || currSensors == 0b00100000 || currSensors == 0b00110000) {
            if ((currSensors == 0b01000000) || (currSensors == 0b01100000)){
              printf("\n\t\t>>> Finished left turn, switching to FAILSAFE");
              turning_sharp = 0;
              switch_state = BLANK_SIGNAL_FAILSAFE;
            };
            break;
          case 2:
            left_ratio = 0.6*base;
            right_ratio = -0.2*base;
            //if (currSensors == 0b00000001 || currSensors == 0b00001000 || currSensors == 0b00000100 || currSensors == 0b00001100) {
            if ((currSensors == 0b00000010) || (currSensors == 0b00000110)) {
              printf("\n\t\t>>> Finished right turn, switching to FAILSAFE");
              turning_sharp = 0;
              switch_state = BLANK_SIGNAL_FAILSAFE;
            };
            break;
      ...
      // Defaults to going straight at blank signals
        curr_weight = 0;
        left_ratio = base;
        right_ratio = base;
      ...
      case LEFT_INTERSECTION:
        if ((EnterLeftInter()) || (turning_sharp == 3)) {
          turning_sharp = 3;
          left_ratio = -0.2*base;
          right_ratio = 0.6*base;
          //if (currSensors == 0b10000000 || currSensors == 0b00010000 || currSensors == 0b00100000 || currSensors == 0b00110000) {
          if ((currSensors == 0b01000000) || (currSensors == 0b01100000)) {
            printf("\n\t\t>>> Finished left intersection, switching to FAILSAFE");
            turning_sharp = 0;
            switch_state = DEFAULT;
          };
        }
      ...
      case RIGHT_INTERSECTION:
        if ((EnterRightInter()) || (turning_sharp == 4)) {
          turning_sharp = 4;
          left_ratio = 0.6*base;
          right_ratio = -0.2*base;
          //if (currSensors == 0b00000001 || currSensors == 0b00001000 || currSensors == 0b00000100 || currSensors == 0b00001100) {
          if ((currSensors == 0b00000010) || (currSensors == 0b00000110)) {
            printf("\n\t\t>>> Finished right intersection, switching to FAILSAFE");
            turning_sharp = 0;
            switch_state = BLANK_SIGNAL_FAILSAFE;
          };
        }
  ...
  // Chương trình sẽ được lặp lại vô tận trong hàm while
    ...
    /*----------Điều khiển xe----------*/
      printf("\tTurning state: %d", turning_sharp);
      SwitchFunction();
